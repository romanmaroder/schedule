...
class m231022_152125_create_schedule_tag_assignments_table extends Migration
{
    ...
    public function up()
    {
        ...
        $this->addPrimaryKey('{{%pk-schedule_tag_assignments}}', '{{%schedule_tag_assignments}}', ['product_id', 'service_id','tag_id']);

        $this->createIndex('{{%idx-schedule_tag_assignments-product_id}}', '{{%schedule_tag_assignments}}', 'product_id');
        $this->createIndex('{{%idx-schedule_tag_assignments-service_id}}', '{{%schedule_tag_assignments}}', 'service_id');
        ...
        $this->addForeignKey('{{%fk-schedule_tag_assignments-service_id}}', '{{%schedule_tag_assignments}}', 'service_id', '{{%schedule_services}}', 'id', 'CASCADE', 'RESTRICT');
