...
class m231021_152339_create_schedule_category_assignments_table extends Migration
{
    ...
    public function up()
    {
        ...
        $this->addPrimaryKey('{{%pk-schedule_category_assignments}}', '{{%schedule_category_assignments}}', ['product_id', 'service_id', 'category_id']);

        $this->createIndex('{{%idx-schedule_category_assignments-product_id}}', '{{%schedule_category_assignments}}', 'product_id');
        $this->createIndex('{{%idx-schedule_category_assignments-service_id}}', '{{%schedule_category_assignments}}', 'service_id');
        ...
        $this->addForeignKey('{{%fk-schedule_category_assignments-service_id}}', '{{%schedule_category_assignments}}', 'service_id', '{{%schedule_services}}', 'id', 'CASCADE', 'RESTRICT');
